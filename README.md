...
| `config.bearer_jwt_auth_types`              | [ 'header' ]                        | false    | Infers the JWT token from different locations: If 'url' is used, the JWT will be inferred from URL query parameters with 'token' as the key. If 'header' is used, the JWT will be inferred from HTTP headers 'Authorization' key, 'Bearer ' prefix is expected here. If 'cookie' is used, the JWT will be inferred from HTTP headers 'Cookie' key. Furthermore, it will look for the cookie value corresponding to `config.bearer_jwt_auth_cookie_name`.
...
| `config.bearer_jwt_auth_cookie_name`        |                                            | false    | When `config.bearer_jwt_auth_types` contains the value "cookie", it will check the Cookie header for the corresponding value of the given cookie name.
