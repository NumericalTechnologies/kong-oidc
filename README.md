...
| `config.jwt_auth_enable`                    | no                                         | false    | Authenticate based on JWT (ID) token provided in Authorization (Bearer) header. Checks iss, sub, aud, exp, iat (as in ID token). `config.discovery` must be defined to discover JWKS                                                                                                                                                                                                                                                              |
| `config.jwt_auth_types`                     | [ 'header' ]                               | false    | Infers the JWT token from different locations: If 'url' is used, the JWT will be inferred from URL query parameters with 'token' as the key. If 'header' is used, the JWT will be inferred from HTTP headers 'Authorization' key, 'Bearer ' prefix is expected here. If 'cookie' is used, the JWT will be inferred from HTTP headers 'Cookie' key. Furthermore, it will look for the cookie value corresponding to `config.jwt_auth_cookie_name`. |
| `config.jwt_auth_allowed_auds`              |                                            | false    | List of JWT token `aud` values allowed when validating JWT token in Authorization header. If not provided, uses value from `config.client_id`                                                                                                                                                                                                                                                                                                     |
| `config.jwt_auth_signing_algs`              | [ 'RS256' ]                                | false    | List of allowed signing algorithms for Authorization header JWT token validation. Must match to OIDC provider and `resty-openidc` supported algorithms                                                                                                                                                                                                                                                                                            |
| `config.jwt_auth_cookie_name`               |                                            | false    | When `config.jwt_auth_types` contains the value "cookie", it will check the Cookie header for the corresponding value of the given cookie name.                                                                                                                                                                                                                                                                                                   |
...
1. Increment kong-oidc-x.y.z-r.rockspec version accordingly

2. Ensure that the `version` is updated according to the new `x.y.z-r` value

3. Run `luarocks upload kong-oidc-x.y.z-r.rockspec --api-key="<api-key>"`
