...
| `config.bearer_jwt_auth_types`              | [ 'url', 'header' ]                        | false    | Infers the JWT token from different locations: If 'url' is used, the JWT will be inferred from URL query parameters with 'token' as the key. If 'header' is used, the JWT will be inferred from HTTP headers 'Authorization' key, 'Bearer ' prefix is expected here.
